...
export default function EditorPage() {
  ...
  const helpText =
    "💡 Поддерживается: ## заголовки, → стрелки, {{переменные}}, CAPS для акцентов";
    ...
    <div className="form-container">
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="form-group">
          ...
          />
          {errors.title && (
            <span className="error-message">⚠️ {errors.title.message}</span>
          )}
        </div>
        ...
        <div className="form-group">
          ...
          />
          {errors.content && (
            <span className="error-message">⚠️ {errors.content.message}</span>
          )}
        </div>
        ...
        <div className="form-group">
          ...
          <input
            {...register("tags")}
            placeholder="например: coding, python, gpt4, ai"
          />
        </div>
        ...
        >
          ...
          <label htmlFor="isPublic" style={{ margin: 0, cursor: "pointer" }}>
            🌍 Опубликовать в публичном каталоге
          </label>
        </div>
